...
def knn(x, k1, k2):
    ...
    with torch.no_grad():
        ...
        try:
            idx = distances.topk(k=k1, dim=-1)[1][:, :, indices]
        except:
            import ipdb;
            ipdb.set_trace()
    return idx
...
def get_graph_feature(x, k1=80, k2=1, idx=None):
    batch_size = x.size(0)
    num_points = x.size(2)
    ...
    feature = feature.view(batch_size, num_points, int(k1/k2), num_dims)
    x = x.view(batch_size, num_points, 1, num_dims).repeat(1, 1, int(k1/k2), 1)

    feature = torch.cat((feature - x, x), dim=3).permute(0, 3, 1, 2)
    return feature

def get_graph_feature_with_normals(x, k1=80, k2=1, idx=None):
    """
...
class DGCNNEncoderGn(nn.Module):
    def __init__(self, knn=80,knn_step=1,if_normals=True):
        super(DGCNNEncoderGn, self).__init__()
        self.k = knn
        self.k_step=knn_step
    ...
    def forward(self, x):
        ...
        # First edge conv
        if self.if_normals:
            x = get_graph_feature_with_normals(x, k1=self.k, k2=self.k_step)
        else:
            x = get_graph_feature(x, k1=self.k, k2=self.k_step)  
        x = self.conv1(x)
        x1 = x.max(dim=-1, keepdim=False)[0]

        # Second edge conv
        x = get_graph_feature(x1, k1=self.k, k2=self.k_step)
        x = self.conv2(x)
        x2 = x.max(dim=-1, keepdim=False)[0]

        # Third edge conv
        x = get_graph_feature(x2, k1=self.k, k2=self.k_step)
        x = self.conv3(x)
...
class PrimitivesEmbeddingDGCNGn(nn.Module):
    ...
    def __init__(self, emb_size=50, num_primitives=8, primitives=False, embedding=False, mode=0,if_normals=True,
                 loss_function=None, knn=80,knn_step=1):
        super(PrimitivesEmbeddingDGCNGn, self).__init__()
        self.mode = mode
        self.if_normals = if_normals
        self.encoder = DGCNNEncoderGn(knn=knn,knn_step=knn_step,if_normals=self.if_normals)
        self.drop = 0.0
    ...
    def forward(self, x, labels, compute_loss=True):
        batch_size = x.shape[0]
        num_points = x.shape[2]
        x, first_layer_features = self.encoder(x)
